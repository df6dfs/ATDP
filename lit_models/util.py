...
def f1_score(output, label, rel_num=42, na_num=13):
    ...
    if sum(guess_by_relation.values()) != 0 and sum(correct_by_relation.values()) != 0:
        ...
        precs = sum(correct_by_relation.values()) / sum(guess_by_relation.values())
        micro_f1 = 2 * recall * precs / (recall+precs)

    return dict(f1=micro_f1, recall=recall, precision=precs,recall_by_relation=recall_by_relation,prec_by_relation=prec_by_relation
