...
def main():
    ...
    for seed in args.seed:
        ...
        print("seed={}下的dataset是{}".format(seed,dataset))
        ...
        label_list = {}
        # print(label_list)
        # print("Other" not in label_list)
        for line in dataset:
            label = line['relation']
            # print("{} not in label_list is {}".format(label,True if label not in label_list else False))
        ...
        with open(os.path.join(setting_dir, "train.txt"), "w") as f:
            ...
            for label in label_list:
                # if label in ['Other','no_relation','NA']:
                #     num = k * (len(label_list)-1)
